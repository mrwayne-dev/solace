/**
 * FILE: /assets/js/admin/funds_xgrid.js
 * ============================================================
 * TitanXHoldings Admin Funds XGrid.js
 * Purpose: Frontend logic for the Admin XGrid Fund Management page.
...
;(function ($) {
    ...
    /** Renders a status badge for XGrid Plans/Contributions. */
    ...
    /**
     * Loads all data for the XGrid dashboard (Metrics, Plans, Allocations).
     */
    async function loadXGridDashboard() {
        ...
        try {
            const res = await fetchApi('/api/admin/funds_xgrid.php', {
            ...
            renderXGridProjectsTable(data.projects); // Table 1: Plans
            renderActiveFundAllocationsTable(data.active_allocations); // Table 2: Contributions
            renderActiveXGridPagination(data.active_page, data.active_total_pages);
    ...
    /**
     * Renders the XGrid Projects table (Plans from DB).
     */
    function renderXGridProjectsTable(projects) {
        const tableBody = $('#infra-projects-body');
        tableBody.empty();

        if (!projects || projects.length === 0) {
            tableBody.html('<tr><td colspan="6" class="text-center text-Gray f14-regular">No X-Grid Plans defined.</td></tr>');
            return;
        }
     ...
     */
    function renderActiveXGridPagination(currentPage, totalPages) {
    ...
    function setupAddProjectModal() {
        $('#infra-project-title').text('Add New X-Grid Plan');
    ...
    function bindInteractions() {
        ...
        $(document).on('click', '.action-toggle-infra-status', async function(e) {
            ...
            const res = await fetchApi('/api/admin/funds_xgrid.php', payload, "POST");
        ...
        $('#infra-project-form').on('submit', async function(e) {
            ...
            window.showToast(`${isEdit ? 'Updating' : 'Creating'} X-Grid Plan...`, 'info', 5000);
    ...
    $(function () {
        ...
        window.refreshXGridDashboard = loadXGridDashboard;
    });
