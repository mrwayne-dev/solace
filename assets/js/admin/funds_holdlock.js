/**
 * FILE: /assets/js/admin/funds_holdlock.js
 * ============================================================
 * HealthRunCare Admin Funds Holdlock.js
 * Purpose: Frontend logic for the Admin HoldLock Savings Management page.
 * Handles: Metrics, HoldLock Plan CRUD, Active HoldLock list/pagination/edit.
...
;(function ($) {
    ...
    /** Renders a status badge for HoldLock (using statuses from the 'holdlock' table). */
    ...
    /**
     * Loads all data for the funds dashboard (Metrics, Plans, Active HoldLock).
     */
    async function loadHoldlockDashboard() {
        ...
        try {
            const res = await fetchApi('/api/admin/funds_holdlock.php', {
    ...
    /**
     * Renders the HoldLock Plans table.
     */
    function renderHoldlockPlansTable(plans) {
        ...
        if (!plans || plans.length === 0) {
            tableBody.html('<tr><td colspan="6" class="text-center text-Gray f14-regular">No HoldLock plans defined.</td></tr>');
            return;
        }
    ...
    /**
     * Renders the Active HoldLock Savings table.
     */
    function renderActiveHoldlockTable(holdlocks) {
        ...
        if (!holdlocks || holdlocks.length === 0) {
            tableBody.html('<tr><td colspan="6" class="text-center text-Gray f14-regular">No HoldLock savings found.</td></tr>');
            return;
        }
    ...
    /**
     * Renders the pagination for the Active HoldLock table.
     */
    ...
    function setupAddPlanModal() {
        $('#holdlock-plan-title').text('Add New HoldLock Plan');
    ...
    /** Fetches plan data and populates the Edit HoldLock Plan modal. */
    ...
    function bindInteractions() {
        ...
        $('#holdlock-plan-form').on('submit', async function(e) {
            ...
            window.showToast(`${isEdit ? 'Updating' : 'Creating'} HoldLock plan...`, 'info', 5000);
            
            const res = await fetchApi('/api/admin/funds_holdlock.php', payload, "POST");
